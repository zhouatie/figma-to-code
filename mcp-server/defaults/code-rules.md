...
1. 按子节点 **Y 坐标排序**
2. Y 坐标接近的节点归为**同一行**（row），容差范围取 **两个节点高度最小值的 50% 与 8px 中的较大值**
3. 每行内按 **X 坐标排序**
4. 整体按行从上到下排列（column）
...
1. **先检查弹窗特征**：如果全屏 FRAME 包含半透明背景层（opacity < 1 或 fills 含透明色），识别为弹窗/底部弹出层
2. **再检查页面特征**：如果全屏 FRAME 含 StatusBar + NavigationBar + 滚动内容，识别为页面
3. **其余**：识别为组件

| Figma 特征                                           | 识别为   | 说明                                       |
| ---------------------------------------------------- | -------- | ------------------------------------------ |
| 全屏 FRAME + 半透明背景层 + 底部圆角内容             | **弹窗** | 生成为 Modal / Sheet 组件                  |
| 全屏 FRAME + 含 StatusBar + NavigationBar + 滚动内容 | **页面** | 生成为 `entries/xxx/index.tsx` 页面入口组件 |
| 非全屏 FRAME / 卡片                                  | **组件** | 生成为 `components/xxx/index.tsx` 子组件    |
...
当 Figma 中出现 **结构相同的同级节点** 时：

-   **2-3 个**：在方案展示阶段标注为"疑似列表"，询问用户是保留静态实现还是改为数据驱动
-   **4 个及以上**：**必须**识别为列表数据，生成 `data.map()` 或 FlatList/FlashList 渲染
-   **禁止**逐个硬编码为静态组件（4 个及以上时）
...
-   列表项是否需要**曝光埋点**
-   在方案阶段询问用户的埋点方案（如有项目专用的埋点组件或 HOC）
